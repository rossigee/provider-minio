...
Managed resources are **namespaced** (`minio.m.crossplane.io/v1beta1`) and require a cluster-scoped `ProviderConfig` (`minio.m.crossplane.io/v1beta1`).
...
Hand-written namespaced examples: `examples/v2/bucket-namespaced.yaml`, `examples/v2/user-namespaced.yaml`, `examples/minio.m.crossplane.io_serviceaccount.yaml`.

`samples/` is split by ownership. These four files are generated by `go generate ./...` from the Go types in `generate_sample.go`, so edit the generator rather than the output:

* `_secret.yaml`
* `minio.m.crossplane.io_bucket.yaml`
* `minio.m.crossplane.io_policy.yaml`
* `minio.m.crossplane.io_providerconfig.yaml`
* `minio.m.crossplane.io_user.yaml`

The remaining files in `samples/` are hand-written and carry explanatory comments and inline PEM material that a typed serializer cannot reproduce, so the generator leaves them alone:

* `minio.m.crossplane.io_providerconfig_with_tls.yaml`
* `minio.m.crossplane.io_serviceaccount.yaml`
* `providerconfig-tls-configmap.yaml`
* `tls-resources.yaml`
